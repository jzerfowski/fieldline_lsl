...
class ApiDataSource(NetworkDataSource):
    ...
    def start_datatype(self, chassis_id, sensor_list, datatype, freq):
        self.configure_datatype(chassis_id, sensor_list, datatype, 1000)

    def stop_datatype(self, chassis_id, sensor_list, datatype):
        self.configure_datatype(chassis_id, sensor_list, datatype, 0)

    def configure_datatype(self, chassis_id, sensor_list, datatype, freq):
    ...
    def start_adc(self, chassis_id, freq):
        self.configure_datatype(chassis_id, [0], 0, 1000)

    def stop_adc(self, chassis_id):
        self.configure_datatype(chassis_id, [0], 0, 0)

    # def _report_data_available(self, sample_list):
    #     self.callback.data_q.put(sample_list)
    ...
    def _parse_frame_data(self, frame_data):
        ...
        for chassis_id, data_packet in frame_data.items():
            ...
            for data_frame in data_packet.data:
                ...
                dt = None
                if 28 == datatype:
                    dt = FieldLineDataType.DATA_BZ_OPEN
                elif 35 == datatype:
                    dt = FieldLineDataType.DATA_BY
                elif 37 == datatype:
                    dt = FieldLineDataType.DATA_BX
                elif 50 == datatype:
                    dt = FieldLineDataType.DATA_BZ_CLOSED
